...
void UGameLiftTutorialGameInstance::SetCognitoTokens(FString NewAccessToken, FString NewIdToken, FString NewRefreshToken)
{
	...
	GetWorld()->GetTimerManager().SetTimer(RetrieveNewTokenHandle, this, &UGameLiftTutorialGameInstance::RetrieveNewTokens, 1.0f, false, 3300.0f);
}
...
void UGameLiftTutorialGameInstance::RetrieveNewTokens()
{
	if (AccessToken.Len() > 0 && RefreshToken.Len() > 0)
	{
		...
		{
			GetWorld()->GetTimerManager().SetTimer(RetrieveNewTokenHandle, this, &UGameLiftTutorialGameInstance::RetrieveNewTokens, 1.0f, false, 30.0f);
		}
	}
}
...
// best to use web sockets for getting ping information
void UGameLiftTutorialGameInstance::GetResponseTime()
{
	...
	GetResponseTimeRequest->OnProcessRequestComplete().BindUObject(this, &UGameLiftTutorialGameInstance::OnGetResonseTimeResponseReceived);
...
void UGameLiftTutorialGameInstance::OnGetResonseTimeResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful)
{
